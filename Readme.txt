...
## Main files:
Each folder contains a Main File from which the user can run all the code files (.do or .sps files) that will code the indicators and produce the tables for that chapter. The user would only need to change the paths in the Main File and the file name for the survey of interest. The user should not delete the globals (macros), and only change to the correct path and survey for their project. 
