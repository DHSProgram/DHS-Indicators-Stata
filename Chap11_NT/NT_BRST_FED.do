...
Date last modified: Dec 16, 2019 by Shireen Assaf 
Note:				
*****************************************************************************************************/
...
/* Tom to provide code for this part *
nt_med_any_bf		"Median duration in months of breastfeeding for any breastfeeding - born last 3yrs"
nt_med_ebf			"Median duration in months of breastfeeding for exclusive breastfeed - born last 3yrs"
nt_med_predo_bf		"Median duration in months of breastfeeding for predominant breastfeeding - born last 3yrs"
nt_mean_any_bf		"Mean duration in months of breastfeeding for any breastfeeding - born last 3yrs"
...
* This routine is called by setup_and_recodes
...
#delimit ;
label define diet 0 "Breastfed Only" 1 "Breastmilk and Water" 2 "Breastmilk and Liquids" 
3 "Breastmilk and Other Milk" 4 "Breastmilk and Solids" 5 "Breastmilk, Milk and Solids" 6 "Weaned" ;
#delimit cr
...
* Define the outcomes for tables 11.3 and 11.4
...
local lbftypes anybf ebf predombf 
foreach lbf of local lbftypes {
gen `lbf'=0
}

* Any breastfeeding
replace anybf=100 if diet<=5
label variable anybf    "Any breastfeeding"

* Exclusive breastfeeding
replace ebf=100 if diet==0
label variable ebf      "Exclusive breastfeeding"

* Predominant breastfeeding
replace predombf=100 if diet<=2 
label variable predombf "Predominant breastfeeding"
...
* ebf=0 and predombf=0 for all children age 24+ months
* anybf=100 if age 24+ months and m4=95

* The data match table 11.3 with no further changes.

* In order to match table 11.4:
...
*   BUT if the lastborn child is living with the mother and has m4=95, set anybf=100 
...
replace anybf=100 if b9==0 & bidx==1 & age_in_months>=24 & m4==95

* MODIFICATION for multiple births.  Not described in the Guide or in the table notes.
* m4=95 only applies to the youngest child.  Sometimes this is the youngest child in a multiple birth, but
*   if there is a multiple birth, all surviving children who live with the mother should share the same bf value.

* Assume that all surviving and coresiding children from a multiple birth have the same bf as the youngest one. 

* This contradicts this note: "It is assumed that...all non-last-born children are not currently breastfeeding". 
...
tab anybf b4 if age_in_months>=24 & age_in_months<=35, m

* transfer that value to any other child in the ntuple who is alive and living with the mother
replace anybf=100 if b0>0 & b0<nch_in_ntuple & stillbf>0 & b9==0 & age_in_months>=24

tab anybf b4 if age_in_months>=24 & age_in_months<=35, m

* In ET70, for example, 8 children are affected.  Their twin is still bf so they should be too.
*list v001 v002 v003 b19 b0 b5 b9 m4 ntuple nch_in_ntuple stillbf anybf if b0>0 & age_in_months>=24, table clean
...
keep if under2==1
save data_for_table_11pt3.dta, replace
...
* Is if possible that the following line should not refer to a child with b9==0 & bidx==1,
*   but to the youngest living child living with the mother?
* replace anybf=100 if b9==0 & bidx==1 & age_in_months>=24 & m4==95
...
replace anybf=0 if age_in_months>=24
replace anybf=100 if youngest_under3_with_mother==1 & age_in_months>=24 & m4==95
*/
...
* If needed for 11.4 it can be recalculated as  nobf=100-anybf.

tab1 *bf,m

drop youngest
save data_for_table_11pt4.dta, replace
...
* The following procedure reproduces the smoothing with a moving average described in the Guide to DHS Statistics. 

* It constructs group2_smoothed, median2_smoothed, and wt_smoothed.

* Runs on the smoothed data should use group2_smoothed in place of group2 (or median2_smoothed in place of median2)
*  and should use wt_smoothed in place of wt_original (usually v005).

* Note: it is not necessary to repeat this smoothing for different outcomes
* v005 is an integer but wt_smoothed (which also includes a factor of 1000000) may not be an integer

* It will still be possible to produce the original distribution using group2 (or median2) and wt.
...
* DHS7+ midpoints
if DHS_phase>=7 {
gen midpoint2_smoothed=2*group2_smoothed-1 if months_since_birth<=35
}
...
tab midpoint2_smoothed ebf [iweight=wt_smoothed/1000000], row
...
* This version of calc_median is for a current status variable which declines with duration, such as EBF or amenorrhea
* For a current status variable it is possible that the decline is not monotonic, so we must
*   find the first value of x for which the percentage having the outcome y is LESS THAN than 50%.
...
tabstat notbf ebf diet1 diet2 diet3 diet4and5 anybf [fweight=v005] if youngest_under2_with_mother==1, statistics(mean) by(`lv') nototal format(%5.1f)
tabstat freq1 freq2, statistics(sum) by(`lv') nototal format(%5.0f)
}


* The exporting of table 11.3 is omitted
...
For this table, use the two-month grouping (group2), as on page 1.46 of the Guide to DHS Statistics,
  with a 3-interval moving total for the numerators and denominators separately.
 Here it is implemented with a revised case-level weight in the routine "smoothed_distribution".

anybf:    any           breastfeeding
ebf:      exclusive     breastfeeding
predombf: predominantly breastfeeding

Table 11.4 includes living and deceased children

*/

use data_for_table_11pt4.dta, clear
...
* The following groups of lines are a clumsy way to get the medians for categories of covariates.
...
*  the widths of the interval, with special term for the first interval.  Description in the 
*  Guide is opaque and should be expanded / clarified
...
* The remainder of this routine is about the construction and exporting of table 11.4
