...
label var fp_demand_tot "Total demand for limiting - total"
...
replace fp_demsat_any=. if inlist(v626a,0,7,8,9) /*eliminate no need from denominator*/
label var fp_demsat_any "Demand satisfied by any methods"

************************
}

* general code for surveys that do not have v626a
if v626a_included==0  {
	...
	* if unmarried and not sexually active in last 30 days, assume no need
	recode v626a .=97 if v502!=1 & sexact!=1
...
* Recode the unmet need variable created above.	
	...
	gen fp_demsat_mod = 0
	replace fp_demsat_mod = 1 if (v626a==3 | v626a==4) & v313==3
	replace fp_demsat_mod = . if v626a>4  /*eliminate no need from denominator*/
	label var fp_demsat_mod "Demand satisfied by modern methods"

	//Demand satisfied by any methods
	gen fp_demsat_any=0
	replace fp_demsat_any=1 if (v626a==3 | v626a==4) 
	replace fp_demsat_any=. if v626a>4  /*eliminate no need from denominator*/
	label var fp_demsat_any "Demand satisfied by any methods"
}
...
if v626a_included==0 & (srvy=="BRIR31" | srvy=="KHIR61" | srvy=="COIR61" | srvy=="CIIR35" | srvy=="GAIR41" | ///
						srvy=="GUIR41" | srvy=="GUIR34" | srvy=="HTIR31" | srvy=="IAIR23" | srvy=="IAIR42" | ///
						srvy=="IAIR52" | srvy=="JOIR42" | srvy=="KKIR42" | srvy=="LSIR61" | srvy=="IAIR42" | ///
						srvy=="MDIR21" | srvy=="MVIR52" | srvy=="MAIR43" | srvy=="NPIR51" | srvy=="NIIR22" | ///
						srvy=="TZIR3A" | srvy=="TZIR41" | srvy=="TZIR63" | srvy=="TRIR31" | srvy=="TRIR41" | ///
						srvy=="TRIR4A" | srvy=="UGIR33" | srvy=="YEIR21" ) {

* open survey again to clear previous variables created in the general code.
